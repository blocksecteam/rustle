...
subgraph Process
    0([Begin])
    1[/Input: source code/]
    2[Scan with regex engine]
    4[Analyze syntax]
    6[Compile with rustc]
    8[Scan with LLVM Pass]

    10[Pattern matching]

    11{Match?}
    12[/Output: vulnerabilities/]
    13([End])
end

subgraph Temporary File
    3[(AST info)]
    5[(Contract struct info)]
    7[(LLVM IR)]
    9[(Contract logic info)]
end
